...
public class GestorJuego {
    ...
    public void reiniciarProgresoBatalla() {
        this.faseActual = 1; // Siempre empezamos en la fase 1 para una nueva run
        ...
        switch (faseActual) {
            case 1:
                ...
                for (int i = 0; i < numEnemigos; i++) {
                    enemigosRestantesFase.add(new Raton("Ratón Común " + (i + 1), 30, 8, 1, "Comun", 10, 20));
                }
                break;
            case 2:
                numEnemigos = 4;
                for (int i = 0; i < numEnemigos; i++) {
                    enemigosRestantesFase.add(new Raton("Ratón Robusto " + (i + 1), 50, 12, 2, "Comun", 15, 30));
                }
                break;
            case 3: // Boss Final
                numEnemigos = 1;
                enemigosRestantesFase.add(new Raton("Gran Jefe Ratón", 150, 25, 5, "Jefe", 50, 100));
                break;
    ...
    // DEBES REEMPLAZAR TU VERSIÓN ANTERIOR DE ESTE MÉTODO POR ESTA.
    public boolean avanzarSiguienteEnemigo() {
        ...
        if (enemigoActual != null && !enemigoActual.estaViva()) { // Asegurarse de que el enemigo fue derrotado
            ...
            if (random.nextInt(100) < 30) {
                ...
                Item pocion = new Item("Poción de Vida Pequeña", "PocionVida", 20, "Cura un poco de vida.", 5);
